...
class Solution():
    def __init__(self, ra_pst):
        ...
        self.init_ra_pst = ra_pst if isinstance(ra_pst, etree._Element) else etree.fromstring(ra_pst)
        self.solution_ra_pst = copy.deepcopy(self.init_ra_pst)
        self.invalid_branches = False  # rename to "is_valid"
        self.is_final = False
        self.allocated_branches = []
        self.ns = {"cpee1": list(self.init_ra_pst.nsmap.values())[
            0], "allo": "http://cpee.org/ns/allocation"}
        self.task_list = self.init_ra_pst.xpath(
    ...
    def get_measure(self, measure, operator=sum, flag=False):  # TODO fix calculation
        ...
        if flag:
            values = self.solution_ra_pst.xpath(
    ...
    def check_validity(self):  # TODO ensure check is done correctly
        ...
        self.solution_ra_pst = etree.fromstring(
            etree.tostring(self.solution_ra_pst))
        tasks = self.solution_ra_pst.xpath(
    ...
    def apply_branches(self, branches: list):
        ...
        while True:
            ...
            node = self.init_ra_pst.xpath(
                f"//*[@id = '{task.attrib['id']}'][not(ancestor::cpee1:children) and not(ancestor::cpee1:allocation) and not(ancestor::RA_RPST)]", namespaces=self.ns)
            ...
            if branch.node.xpath("//*[@type='delete']"):
                ...
                self.solution_ra_pst = branch.apply_to_process(
                    self.solution_ra_pst, solution=self)  # build branch
            ...
            if self.current_task == "end":
                for branch, task in self.delay_deletes:
                    if self.solution_ra_pst.xpath(f"//*[@id='{task.attrib['id']}'][not(ancestor::cpee1:children) and not(ancestor::cpee1:allocation) and not(ancestor::RA_RPST)]", namespaces=self.ns):
                        self.solution_ra_pst = branch.apply_to_process(
                            self.solution_ra_pst, solution=self)  # apply delays
                self.is_final = True
                break
            try:
                branch_no = branches[i]
            except:
                # TODO: All deletes are done in the end -> how to calculate this in reward?
                break
        ...
        if node.tag == f"{{{self.ns['cpee1']}}}resprofile" or (node.tag == f"resprofile"):
            ...
            if not children and node_type == 'delete':
                ...
                task_labels = [utils.get_label(etree.tostring(
                    task)).lower() for task in self.task_list]
    ...
    def get_pickleable_object(self):
        ...
        solution.ns = {"cpee1": list(self.init_ra_pst.nsmap.values())[
            0], "allo": "http://cpee.org/ns/allocation"}
        solution.task_list = []
    ...
    def transform_to_valid_only_ra_pst(self):
        """Returns a new Solution object, with an ra_pst that only consists of valid branches"""

        valid_solution = Solution(self.init_ra_pst)

        for task in valid_solution.init_ra_pst.xpath(
                "(//cpee1:call|//cpee1:manipulate)[not(ancestor::cpee1:children) and not(ancestor::cpee1:allocation)]", namespaces=self.ns):
            branches = valid_solution.get_branches_for_task_wrap(task)
            for branch in branches:
                # if branch.is_valid == False: remove branch from children
                if not branch.is_valid:
                    to_del_id = branch.node.xpath(
                        "cpee1:children/*", namespaces=valid_solution.ns)[0].attrib['id']
                    to_del_branch = task.xpath(
                        f"cpee1:children/*[@id = '{to_del_id}']", namespaces=valid_solution.ns)[0]
                    to_del_branch.getparent().remove(to_del_branch)

            if not task.xpath("cpee1:children/cpee1:resource", namespaces=valid_solution.ns):
                raise NoValidSolutionError(f"For the task {utils.get_label(task)}, no valid solution is available")

        return Solution(valid_solution.init_ra_pst)

class NoValidSolutionError(Exception):
